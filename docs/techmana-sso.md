...
同じ理由で、**リフレッシュ直後にセーブAPIが落ちた場合も**回した新しい
トークンを例外に載せて返し、呼び出し側が Cookie を貼り直す。これを
落とすとブラウザが消費済みのトークンを持ち続け、次の要求で再提示して
同じ事故を起こす。

なお `SESSION_SECRET` が未設定のときは、警告を出すだけでなく **連携機能
そのものを無効として報告する**。鍵が毎起動ランダムでは Cookie が再起動
のたび復号不能になり、利用者からは「連携が勝手に切れる」としか見えない。
そうなるくらいなら機能を出さない(ゲーム本体は従来どおり動く)。
...
v9 で別方式の連携(起動トークン)が入ったため、現在は2つの入口が共存している。

| | OAuth ログイン(本書の対象) | 起動トークン(v9) |
| --- | --- | --- |
| 入口 | ロビーの「テクマナでログイン」 | テクマナが `?tm_token=` 付きでゲームを開く |
| 検証 | 認可コード + PKCE をサーバ間で交換 | `TECHMANA_VERIFY_URL` へ POST |
| 状態 | **稼働中**(本番で検証済み) | **無効**(環境変数未設定) |
| 得られるもの | 同定 + セーブの読み書き | 同定のみ |

起動トークン方式が要求する検証エンドポイントは、現時点のテクマナには
存在しない(あるのは Bearer 認証の `/api/v1/me` と `/api/v1/saves/*` のみ)。
`packages/server/src/techmana.ts` のコメントどおり「正式仕様が確定したら
合わせる」段階の仮実装で、`TECHMANA_VERIFY_URL` が未設定のあいだは
`isTechmanaEnabled()` が false を返して何も起きない。

両方とも `socket.data.account` をサーバ側で立てるところに統一してある。
どちらの経路で入っても、同定キーはクライアントから書き換えられない。
...
ゲスト側も `guest:` 名前空間に押し込んでいる。押し込まないとゲストが
`tm:777` を名乗れてしまい、ログイン済みの人のランキング行に書き込める
(`getOrCreatePlayer` は fingerprint の文字列一致だけで行を引き当て、
ニックネームまで上書きする)。16進以外を捨てるので `tm:` は構文的に
作れない。
